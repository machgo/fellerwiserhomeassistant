...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    task = hass.async_create_task(hello(covers, hass, host, apikey))
...
class FellerCover(CoverEntity):
    ...
    def update(self) -> None:
        ...
        self._position = int(100 - (load["data"]["state"]["level"] / 100))

        moving = load["data"]["state"]["moving"]
        self._is_opening = moving == "up"
        self._is_closing = moving == "down"
        self._is_closed = self._position <= 0
    
    def updateExternal(self, position, moving):
        self._position = int(100 - (position / 100))
        self._is_opening = moving == "up"
        self._is_closing = moving == "down"
        self._is_closed = self._position <= 0
